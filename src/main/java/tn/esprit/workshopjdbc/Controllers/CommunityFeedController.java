...
public class CommunityFeedController {
    ...
    private VBox createPostCard(ForumPost post) {
        ...
        translateBtn.getStyleClass().add("interaction-button");
        translateBtn.setStyle("-fx-font-size: 11px; -fx-padding: 6 12;");
    ...
    private void showStoryCaption(File imageFile) {
        ...
        if (result.isPresent() && result.get() == javafx.scene.control.ButtonType.OK) {
            saveStory(imageFile, captionField.getText());
        }
    }
    ...
    private void showStory(Story story) {
        ...
        dialog.getDialogPane().setPrefSize(550, 700);
    ...
    private void handleTranslatePost(ForumPost post) {
        ...
        languageCombo.getItems().addAll("English (EN)", "Français (FR)", "العربية (AR)");
        ...
        var result = dialog.showAndWait();
        if (result.isPresent() && result.get() == javafx.scene.control.ButtonType.OK) {
            String targetLanguage = languageCombo.getValue();
        ...
        new Thread(() -> {
            ...
            Platform.runLater(() -> {
                if (titleResult.success && contentResult.success) {
                    ...
                    showError("Translation failed: " + (titleResult.error != null ? titleResult.error : contentResult.error));
                }
            });
        }).start();
    }
